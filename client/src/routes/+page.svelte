<script lang="ts">
	...
	let translated = {
		...
		source_language: 'Sinhala (Sinhalese)',
		trust_level: 1
	};
	...
	const convertCurrency = async () => {
		const url =
			`https://currency-conversion-and-exchange-rates.p.rapidapi.com/convert?from=${from}&to=LKR&amount=${currency}`;
		const options = {
			method: 'GET',
			headers: {
				'X-RapidAPI-Key': '9e333c0431mshd8d87c23fe0cd6ep1d43f8jsn94680954db83',
		...
		try {
			const response = await fetch(url, options);
			const result = await response.text();
			convertedCurrency = JSON.parse(result);
			console.log(result);
		} catch (error) {
			console.error(error);
		}
	...
	<div class="p-5 md:p-10">
		...
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
			...
			<div class="card rounded-lg bg-white p-5 shadow">
				...
				<p class="my-4">{translated.trans}</p>
				<div class="flex flex-col items-center gap-4 space-x-2">
					<input
						type="text"
						class="w-full rounded-md border border-gray-300 p-2"
						placeholder="Enter text to translate"
						bind:value={translate}
					/>
					<button on:click={toEng} class="w-full rounded-md bg-blue-500 px-3 py-1 text-white"
						>To EN</button
					>
					<button on:click={fromEng} class="w-full rounded-md bg-blue-500 px-3 py-1 text-white"
						>From EN</button
					>
				</div>
			</div>
